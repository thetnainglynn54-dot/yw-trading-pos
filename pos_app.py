...
def make_stock_report(source_df, report_start, report_end, brand="All", category="All", item="All"):
    if source_df is None or source_df.empty:
        return pd.DataFrame()

    report_df = source_df.copy()
    report_df["Date"] = pd.to_datetime(report_df["Date"], errors="coerce").dt.date
    report_df = report_df.dropna(subset=["Date"])

    for col in ["Before Amt", "Purchase Qty", "Pur Price", "Sale Qty", "Sale Price", "Stock", "Balance"]:
        report_df[col] = pd.to_numeric(report_df[col], errors="coerce").fillna(0.0)

    if brand != "All":
        report_df = report_df[report_df["Brand"].astype(str) == str(brand)]
    if category != "All":
        report_df = report_df[report_df["Category"].astype(str) == str(category)]
    if item != "All":
        report_df = report_df[report_df["Item"].astype(str) == str(item)]

    if report_df.empty:
        return pd.DataFrame()

    if "Original_Index" not in report_df.columns:
        report_df["Original_Index"] = report_df.index

    period_df = report_df[(report_df["Date"] >= report_start) & (report_df["Date"] <= report_end)].copy()
    up_to_end_df = report_df[report_df["Date"] <= report_end].copy()
    before_start_df = report_df[report_df["Date"] < report_start].copy()

    rows = []
    product_keys = unique_product_keys(up_to_end_df)
    for product_key in product_keys:
        b_name, c_name, i_name = product_key
        product_mask = (
            (report_df["Brand"].astype(str) == b_name)
            & (report_df["Category"].astype(str) == c_name)
            & (report_df["Item"].astype(str) == i_name)
        )

        end_rows = up_to_end_df[product_mask].sort_values(
            by=["Date", "Original_Index"],
            ascending=[True, True],
            kind="mergesort",
        )
        if end_rows.empty:
            continue
        latest_row = end_rows.iloc[-1]

        opening_rows = before_start_df[product_mask].sort_values(
            by=["Date", "Original_Index"],
            ascending=[True, True],
            kind="mergesort",
        )
        opening_stock = float(opening_rows.iloc[-1]["Stock"]) if not opening_rows.empty else 0.0

        period_rows = period_df[product_mask]
        purchase_qty = float(period_rows["Purchase Qty"].sum()) if not period_rows.empty else 0.0
        sale_qty = float(period_rows["Sale Qty"].sum()) if not period_rows.empty else 0.0
        closing_stock = float(latest_row["Stock"])
        latest_purchase_price = float(latest_row["Pur Price"])
        latest_balance = float(latest_row["Balance"])
        stock_value = latest_balance if latest_balance else closing_stock * latest_purchase_price
        ...
        rows.append({
            ...
            "Latest Pur Price": latest_purchase_price,
            "Latest Sale Price": float(latest_row["Sale Price"]),
            "Last Transaction Date": latest_row["Date"],
            "Generated By": st.session_state.get("current_user", "Unknown"),
            "Generated At": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        })

    result = pd.DataFrame(rows)
    if not result.empty:
        result = result.sort_values(by=["Brand", "Category", "Item"], kind="mergesort").reset_index(drop=True)
    return result


def dataframe_to_excel_bytes(report_df):
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        report_df.to_excel(writer, index=False, sheet_name="Stock Report")
    output.seek(0)
    return output.getvalue()


def render_printable_stock_report(report_df, title):
    if report_df.empty:
        return
    printable_df = report_df.copy()
    html_table = printable_df.to_html(index=False, escape=False)
    generated_by = st.session_state.get("current_user", "Unknown")
    ...
    <body>
        <h2>{title}</h2>
        <div class="meta">Generated by {generated_by} at {generated_at}</div>
        {html_table}
        <script>
